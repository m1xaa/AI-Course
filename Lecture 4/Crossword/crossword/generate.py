...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        """
        copy_domains = self.domains[x].copy()
        revised = False
        (i, j) = self.crossword.overlaps[x, y]
        for word_x in self.domains[x]:
            if all(word_x[i] != word_y[j] for word_y in self.domains[y]):
                copy_domains.remove(word_x)
                revised = True
        self.domains[x] = copy_domains
        return revised
    
    def find_initial_arcs(self):
        arcs = Queue()
            ...
            for neighbor in neighbors:
                ...
                for value_neighbor in self.domains[neighbor]:
                    if value[i] != value_neighbor[j]:
                        eliminated += 1
            num_eliminated[value] = eliminated
        
        sorted_keys = sorted(num_eliminated, key=lambda k: num_eliminated[k])
        return sorted_keys
    ...
    def select_unassigned_variable(self, assignment):
        ...
        return values.
        """
        min_remaining_value = len(self.crossword.words)
        vars = []
        for var in self.crossword.variables:
            if var in assignment: continue
            if len(self.domains[var]) == min_remaining_value:
                vars.append(var)
            if len(self.domains[var]) < min_remaining_value:
                vars.clear()
                vars.append(var)
                min_remaining_value = len(self.domains[var])
        if len(vars) > 1:
            max_num_neighbors = 0
            return_vars = []
            for var in vars:
    ...
    def backtrack(self, assignment: dict):
        ...
        for value in self.order_domain_values(variable, assignment):
            ...
            if self.consistent(assignment_copy):
                ...
                arcs = Queue()
                arcs.put((neighbor, variable) for neighbor in neighbors)
                self.ac3()
